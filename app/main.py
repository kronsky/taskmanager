...
def notification():
    while True:
        ...
        for table in sqlrequest.get_tables():
...
@bot.message_handler(state=MyStates.title)
def get_title(message):
    bot.send_message(message.chat.id, 'Введите описание задачи')
    bot.set_state(message.from_user.id, MyStates.description, message.chat.id)
...
def get_description(message):
    ...
    bot.set_state(message.from_user.id, MyStates.tag, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['description'] = message.text


@bot.message_handler(state=MyStates.tag)
def get_tag(message):
    menu = types.ReplyKeyboardMarkup(resize_keyboard=True)
    ...
    bot.set_state(message.from_user.id, MyStates.reminder, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['tag'] = message.text


@bot.message_handler(state=MyStates.reminder, is_digit=True)
def get_reminder(message):
    menu = date_button()
    bot.send_message(message.chat.id, 'Введите дату начала задачи в формате гггг-мм-дд или',
                     reply_markup=menu)
    bot.set_state(message.from_user.id, MyStates.dt_start_d, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['reminder'] = message.text


@bot.message_handler(state=MyStates.dt_start_d, is_date=True)
def get_dt_start(message):
    menu = time_button()
    bot.send_message(message.chat.id, 'Введите время начала задачи в формате чч:мм',
                     reply_markup=menu)
    bot.set_state(message.from_user.id, MyStates.dt_start, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['dt_start_d'] = message.text


@bot.message_handler(state=MyStates.dt_start, is_time=True)
def get_dt_start(message):
    menu = date_button()
    bot.send_message(message.chat.id, 'Введите дату крайнего срока в формате гггг-мм-дд',
                     reply_markup=menu)
    bot.set_state(message.from_user.id, MyStates.dt_deadline_d, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['dt_start'] = data['dt_start_d'] + ' ' + message.text
        data['dt_start'] = datetime.strptime(data['dt_start'], '%Y-%m-%d %H:%M')


@bot.message_handler(state=MyStates.dt_deadline_d, is_date=True)
def get_dt_start(message):
    menu = time_button()
    bot.send_message(message.chat.id, 'Введите время крайнего срока в формате чч:мм',
                     reply_markup=menu)
    bot.set_state(message.from_user.id, MyStates.dt_deadline, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['dt_deadline_d'] = message.text


@bot.message_handler(state=MyStates.dt_deadline, is_time=True)
...
@bot.message_handler(state=MyStates.reminder, is_digit=False)
def reminder_incorrect(message):
    bot.send_message(message.chat.id, 'Вы ввели не число, попробуйте еще раз')


@bot.message_handler(state=MyStates.dt_start_d, is_date=False)
...
@bot.message_handler(state=MyStates.dt_deadline_d, is_date=False)
def dt_deadline_d_incorrect(message):
    bot.send_message(message.chat.id, 'Неверный формат даты, введите дату в формате гггг-мм-дд '
                                      'или нажмите на кнопку')


@bot.message_handler(state=MyStates.dt_start, is_date=False)
def dt_start_incorrect(message):
    bot.send_message(message.chat.id, 'Неверный формат времени, введите время в формате чч:мм '
                                      'или нажмите на кнопку')


@bot.message_handler(state=MyStates.dt_deadline, is_date=False)
